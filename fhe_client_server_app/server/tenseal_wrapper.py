"""
TenSEAL Wrapper for FHE Operations
...
class TenSEALWrapper:
    ...
    def encrypt_data(self, data: List[Any], column_name: str, data_type: str) -> List[Optional[bytes]]:
        ...
        if not self.context:
            raise ValueError("Context not initialized. Call generate_context() first.")

        if self.scheme == 'BFV' and data_type == 'text':
            raise ValueError(
                "BFV scheme does not support text encryption directly. "
                "Use CKKS or encode text as integers."
            )
        ...
        for value in data:
            try:
                ...
                if data_type == 'numeric':
                    ...
                    # For text, convert to numeric encoding (ASCII sum)
    ...
    def get_scheme_limitations(self) -> Dict[str, Any]:
        ...
        limitations = {
            'CKKS': {
                ...
                'disadvantages': ['Approximate results', 'No exact integers']
            },
            'BFV': {
                ...
                'disadvantages': ['No floating point', 'Limited text support']
            }
        }
