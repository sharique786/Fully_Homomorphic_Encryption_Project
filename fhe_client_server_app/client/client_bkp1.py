    ...
    if st.session_state.restricted_data_detected:
        ...
        with st.expander("⚠️ Restricted Data Details", expanded=True):
            if st.session_state.df_users is not None and 'is_restricted' in st.session_state.df_users.columns:
                ...
                st.error("⚠️ **Processing Requirement:** All data from these countries must be processed on-premises only")
    ...
    with col1:
        ...
        if data_source == "Generate Synthetic Data":
            with st.expander("🎲 Generate Synthetic Data", expanded=True):
                ...
                with col_c:
                    num_transactions = st.number_input("Number of Transactions", min_value=50, max_value=10000, value=1000)
...
def encrypt_selected_columns(batch_size):
    ...
    try:
        ...
        for table_name, df_key in [('users', 'df_users'), ('accounts', 'df_accounts'), ('transactions', 'df_transactions')]:
