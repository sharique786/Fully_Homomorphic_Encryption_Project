...
def render_results_statistics_tab():
    ...
    if st.session_state.analysis_results:
        ...
        with st.expander("📊 Detailed Results", expanded=True):
            # Summary metrics based on operation type
            if results.get('operation_type') or 'total_transactions' in results:
                operation = results.get('operation_type', 'Unknown')

                # Transaction Analysis
                if 'transactions_detail' in results and results['transactions_detail']:
                    ...
                    df_tx = pd.DataFrame(results['transactions_detail'])

                    # Add formatting
                    df_tx_display = df_tx.copy()
                    df_tx_display['amount'] = df_tx_display['amount'].apply(lambda x: f"${x:,.2f}")

                    st.dataframe(
                        df_tx_display[['transaction_id', 'date', 'account_id', 'amount',
                                       'currency', 'transaction_type', 'merchant',
                                       'credited_party', 'status']],
                    ...
                    st.markdown("#### 📈 Transaction Amount Timeline")
                ...
                # Account Summary
                elif 'accounts_detail' in results and results['accounts_detail']:
                    ...
                    with col4:
                        st.metric("Avg Balance", f"${results.get('avg_balance', 0):,.2f}")

                    # Detailed accounts table
                    st.markdown("#### 💳 Account Details")
                    df_acc = pd.DataFrame(results['accounts_detail'])

                    # Add formatting
                    df_acc_display = df_acc.copy()
                    df_acc_display['balance'] = df_acc_display['balance'].apply(lambda x: f"${x:,.2f}")

                    st.dataframe(
                        df_acc_display[['account_id', 'account_type', 'balance', 'currency',
                                        'status', 'created_date', 'last_transaction', 'transaction_count']],
                        use_container_width=True,
                        height=300
                    )
                    ...
                    with col_a:
                        ...
                        if 'account_types' in results:
                            df_types = pd.DataFrame([
                                {'Type': k, 'Count': v}
                                for k, v in results['account_types'].items()
                            ])
                            fig1 = px.bar(
                                df_types,
                                x='Type',
                    ...
                    with col_b:
                        # By currency
                        if 'currency_totals' in results:
                            df_currency = pd.DataFrame([
                                {'Currency': k, 'Balance': v}
                                for k, v in results['currency_totals'].items()
                            ])
                            fig2 = px.pie(
                                df_currency,
                                values='Balance',
                    ...
                    status_count = df_acc['status'].value_counts()
                    fig3 = px.bar(
                ...
                # Standard metrics for other operations
                else:
            ...
            # Currency breakdown (if available and not already shown)
            if 'currency_breakdown' in results and 'transactions_detail' not in results:
            ...
            # Monthly pattern (if available and not already shown)
            if 'monthly_pattern' in results and 'transactions_detail' not in results:
            ...
            # Transaction type distribution (if available and not already shown)
            if 'transaction_type_distribution' in results and 'transactions_detail' not in results:
...
def render_encryption_analysis_tab():
    ...
    # Show currently encrypted data info
    if st.session_state.encrypted_data:
        st.subheader("🔐 Currently Encrypted Data")

        # Count encrypted columns by table
        encrypted_by_table = {}
        for key, data in st.session_state.encrypted_data.items():
            table = data.get('table', key.split(':')[0])
            if table not in encrypted_by_table:
                encrypted_by_table[table] = []
            encrypted_by_table[table].append(data)

        # Display summary
        col1, col2, col3 = st.columns(3)
        with col1:
            users_count = len(encrypted_by_table.get('users', []))
            st.metric("Users Columns", users_count)
        with col2:
            accounts_count = len(encrypted_by_table.get('accounts', []))
            st.metric("Accounts Columns", accounts_count)
        with col3:
            transactions_count = len(encrypted_by_table.get('transactions', []))
            st.metric("Transactions Columns", transactions_count)

        # Show detailed encrypted columns
        with st.expander("📋 View Encrypted Columns Details", expanded=False):
            for table_name in ['users', 'accounts', 'transactions']:
                if table_name in encrypted_by_table:
                    st.markdown(f"**{table_name.upper()} Table:**")

                    cols_info = []
                    for enc_data in encrypted_by_table[table_name]:
                        cols_info.append({
                            'Column': enc_data.get('column', 'Unknown'),
                            'Type': enc_data.get('data_type', 'Unknown'),
                            'Records': enc_data.get('encrypted_count', 0),
                            'Scheme': enc_data.get('scheme', 'Unknown'),
                            'Library': enc_data.get('library', 'Unknown')
                        })

                    if cols_info:
                        df_cols = pd.DataFrame(cols_info)
    ...
    try:
        ...
        for table_name, df_key in [('users', 'df_users'), ('accounts', 'df_accounts'),
            ...
            for column in selected_cols:
                ...
                encryption_summary.append({
                    'table': table_name,
                    'column': column,
                    'records': len(encrypted_values),
                    'time': round(encryption_time, 2),
                    'scheme': scheme,
                    'library': library
                })
        ...
        if encryption_summary:
            ...
            df_summary = pd.DataFrame(encryption_summary)
            st.dataframe(df_summary, use_container_width=True)

        # Display encrypted data samples by table
        st.markdown("---")
        st.markdown("### 🔐 Encrypted Data Preview")
        st.info("👉 Showing first 5 encrypted records per column (collapsed by default)")
        ...
        # Display each table's encrypted columns
        for table_name in ['users', 'accounts', 'transactions']:
            if table_name in encrypted_by_table:
                with st.expander(
                        f"🔒 {table_name.upper()} Table - {len(encrypted_by_table[table_name])} columns encrypted",
                        expanded=False):
                    for enc_data in encrypted_by_table[table_name]:
                        column_name = enc_data['column']
                        encrypted_values = enc_data['encrypted_values']

                        st.markdown(f"**Column: `{column_name}`**")
                        st.text(
                            f"Type: {enc_data['data_type']} | Scheme: {enc_data['scheme']} | Library: {enc_data['library']}")
                        st.text(
                            f"Total encrypted: {enc_data['encrypted_count']} records | Time: {enc_data['encryption_time']:.2f}s")

                        # Show first 5 encrypted records
                        sample_size = min(5, len(encrypted_values))
                        if sample_size > 0:
                            st.markdown(f"*Sample (first {sample_size} records):*")

                            # Create a display format based on encryption type
                            sample_data = []
                            for i in range(sample_size):
                                enc_val = encrypted_values[i]
                                if enc_val is None:
                                    display_val = "NULL"
                                elif isinstance(enc_val, dict):
                                    if 'ciphertext' in enc_val:
                                        display_val = enc_val['ciphertext'][:50] + "..." if len(
                                            str(enc_val['ciphertext'])) > 50 else enc_val['ciphertext']
                                    else:
                                        display_val = str(enc_val)[:50] + "..."
                                elif isinstance(enc_val, bytes):
                                    display_val = enc_val.hex()[:50] + "..."
                                else:
                                    display_val = str(enc_val)[:50] + "..."

                                sample_data.append({
                                    'Index': i,
                                    'Encrypted Value': display_val,
                                    'Type': enc_data['data_type']
                                })

                            df_sample = pd.DataFrame(sample_data)
                            st.dataframe(df_sample, use_container_width=True, height=200)
