...
def screen_2_encryption():
    ...
    if st.button("🔒 Encrypt Selected Columns", type="primary", disabled=len(all_selected) == 0):
        ...
        for idx, col in enumerate(all_selected):
            ...
            if result and result.get('status') == 'success':
                ...
                # FIX: Handle both individual and packed_vector modes
                if simd_mode == "packed_vector":
                    # Packed vector returns batches
                    for enc_result in encrypted_results:
                        ...
                        # Store packed vector with batch metadata
                        conn = st.session_state.db_conn
                        cursor = conn.cursor()
                        record_id = str(uuid.uuid4())
                    ...
                    # Individual or batch_processing modes
                    for enc_result in encrypted_results:
                        encrypted_value = enc_result.get('encrypted_value')
    ...
    if st.session_state.encrypted_metadata:
        ...
        # FIX 1: Add encrypted data preview with search
        st.divider()
        ...
        if search_email:
            cursor.execute("""
                SELECT email_id, column_name, encrypted_value, original_value, data_type 
                FROM encrypted_data 
                WHERE email_id LIKE ? 
                LIMIT ?
        ...
        if preview_data:
            ...
            # Truncate encrypted value for display
            ...
            st.caption(f"Showing {len(preview_data)} encrypted records with original values for verification")
        else:
            ...
            if st.button("🔓 Decrypt Fraud Score"):
                with st.spinner("Decrypting..."):
                    ...
                    if encrypted_score is None:
                        st.error("❌ Fraud score is None. Detection may have failed.")
                        ...
                        st.session_state.decrypted_fraud_score = fraud_score if fraud_score is not None else 0.0
    ...
    if st.button("🚀 Run ML Inference", type="primary"):
        with st.spinner("Running ML inference on encrypted features..."):
            ...
            for feature_name in selected_features:
                cursor.execute(
                    "SELECT encrypted_value FROM encrypted_data WHERE column_name = ? AND party_id != 'BATCH_' LIMIT 1",
                    (feature_name,)
                )
                ...
                if result:
                    ...
                    if isinstance(enc_value, bytes):
                        original_size += len(enc_value)

                    # Compress encrypted value
                    if isinstance(enc_value, bytes):
                        compressed = gzip.compress(enc_value, compresslevel=6)
                        compressed_size += len(compressed)
                        encrypted_features_list.append(base64.b64encode(compressed).decode())
                    else:
                        encrypted_features_list.append(enc_value)
                else:
                    st.error(f"No encrypted data found for feature: {feature_name}")
                    return

            if len(encrypted_features_list) != len(selected_features):
                st.error(
                    f"Could not retrieve all encrypted features. Got {len(encrypted_features_list)}, expected {len(selected_features)}")
                return
            ...
            if original_size > 0 and compressed_size > 0:
                ...
                st.info(
                    f"📦 Compression: {original_size:,} → {compressed_size:,} bytes ({compression_ratio:.1f}% reduction)")
