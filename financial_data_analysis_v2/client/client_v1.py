...
def screen_1_data_upload():
    ...
    with col1:
        if check_server_health():
            ...
            st.info("Start client: `python client_v1.py`")
            return
                        ...
                        if st.button(f"Decrypt {operation.upper()}", key=f"decrypt_{column_name}_{operation}",
                            ...
                            with st.spinner(f"Decrypting {operation}..."):
                                ...
                                if decrypt_result and decrypt_result.get('status') == 'success':
                                    raw_value = decrypt_result.get('decrypted_value')
                                    # Normalize decrypted value to a float when possible, otherwise keep raw for display
                                    decrypted_value = None
                                    try:
                                        if isinstance(raw_value, (bytes, bytearray)):
                                            # decode bytes then try to parse
                                            s = raw_value.decode('utf-8')
                                            decrypted_value = float(s)
                                        elif isinstance(raw_value, (list, tuple)) and len(raw_value) > 0:
                                            # take first element if list-like
                                            decrypted_value = float(raw_value[0])
                                        else:
                                            decrypted_value = float(raw_value)
                                    except Exception:
                                        decrypted_value = None

                                    expected_value = result.get('expected_value')

                                    # Calculate difference and match status only when both are numeric
                                    if expected_value is not None and isinstance(expected_value, (int,
                                                                                                  float)) and decrypted_value is not None:
                                    ...
                                    reconciliation_entry = {
                                        ...
                                        'Expected Value': fmt_currency(expected_value),
                                        'Computed Value (FHE)': fmt_currency(
                                            decrypted_value) if decrypted_value is not None else (
                                    ...
                                    # Display success safely (avoid numeric formatting on non-numeric values)
                                    if isinstance(decrypted_value, (int, float)):
                                        st.success(f"✅ {decrypted_value:,.2f}")
                                    elif raw_value is not None:
                                        st.success(f"✅ {raw_value}")
                                    else:
