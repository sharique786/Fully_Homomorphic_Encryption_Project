"""
Additional Streamlit Screens for Enhanced FHE Features
Add these to your existing client_4.py
Screens: ML Inference, Fraud Detection, SIMD Operations, Parameter Selection
"""
                ...
                if not records_df.empty:
                    ...
                    if st.checkbox(f"Show detailed ciphertext for first record in {col}", key=f"detail_{col}"):
                        if len(records_df) > 0:
                            ...
                            st.caption(f"Note: Full ciphertext is {first_record.get('ciphertext_full_size', 0)} bytes")
                else:
...
def screen_5_parameter_selection():
    """Parameter auto-selection and optimization"""
    st.title("⚙️ Parameter Selection & Optimization")
    st.markdown("### Automatic FHE Parameter Recommendation")

    st.info("💡 Get optimized parameters based on your workload type")
    ...
    with col2:
        st.markdown("**Workload Descriptions:**")
        workload_descriptions = {
            "transaction_analytics": "Basic sum, avg, count operations. Shallow depth, high throughput.",
            "fraud_scoring": "ML-based scoring with weighted features. Medium depth circuits.",
            "ml_inference": "Deep neural networks, complex models. Requires bootstrapping.",
            "exact_comparison": "Integer operations, counting, exact equality tests.",
            "high_precision": "60-bit precision for financial calculations."
        }
        st.info(workload_descriptions.get(workload_type, ""))

    # Optional: Specify expected operations
    with st.expander("🔍 Advanced: Specify Operation Sequence"):
        st.markdown("Estimate multiplicative depth based on operations:")

        operations_input = st.text_area(
            "Operations (comma-separated)",
            value="add,multiply,add,multiply",
            help="Example: add,multiply,add,multiply,square"
        )

        if operations_input:
            expected_ops = [op.strip() for op in operations_input.split(',')]
        else:
            expected_ops = None

    if st.button("🎯 Get Recommendations", type="primary"):
        with st.spinner("Analyzing workload and computing optimal parameters..."):
            request_data = {
                "workload_type": workload_type,
                "security_level": security_level,
                "expected_operations": expected_ops if operations_input else None
            }
            ...
            if result and result.get('status') == 'success':
                ...
                st.success("✅ Parameters Computed Successfully!")

                # Key parameters in metrics
                col1, col2, col3, col4 = st.columns(4)
                ...
                # Full parameters
                with st.expander("📋 Complete Parameter Set", expanded=True):
                    st.json(params)

                # Depth estimation
                if 'estimated_depth' in params:
                    st.info(f"📊 Estimated Multiplicative Depth: {params['estimated_depth']}")

                # Usage instructions
                st.divider()
                st.subheader("📝 How to Use These Parameters")

                code_example = f"""
# Use these parameters when generating context:

context_config = {{
    "library": "TenSEAL",  # or "OpenFHE"
    "scheme": "{params.get('scheme', 'CKKS')}",
    "poly_modulus_degree": {params.get('poly_modulus_degree', 8192)},
    "mult_depth": {params.get('mult_depth', 10)},
    "scale": {params.get('scale', 2 ** 40)},
    "scale_mod_size": {params.get('scale_mod_size', 40)}
}}

# Call /generate_context with these parameters
"""
                ...
                st.download_button(
                    ...
                    data=params_json,
                    file_name=f"fhe_params_{workload_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
                    mime="application/json"
                )
...
def screen_7_fraud_detection():
    ...
    with st.expander("⚙️ Alert Thresholds"):
        low_risk_threshold = st.number_input("Low Risk Threshold", value=0.3)
        medium_risk_threshold = st.number_input("Medium Risk Threshold", value=0.6)
        high_risk_threshold = st.number_input("High Risk Threshold", value=0.8)
    ...
    if st.button("🔍 Run Fraud Detection", type="primary"):
        with st.spinner(f"Analyzing encrypted transactions for fraud..."):
            try:
                # Mock encrypted transaction
                encrypted_transaction = {}
                for feature in selected_features:
                    # In real scenario, use actual encrypted bytes
                    encrypted_transaction[feature] = base64.b64encode(b"mock_encrypted").decode()
                ...
                if result and result.get('status') == 'success':
                    ...
                    with col1:
                        st.metric("Method", detection_type.replace('_', ' ').title())
                    with col2:
                    ...
                    if st.button("🔓 Decrypt & Analyze Fraud Score"):
                        ...
                        if decrypt_result and decrypt_result.get('status') == 'success':
                            ...
                            if risk_level == "LOW":
                                ...
                                st.error(
                                    "🚨🚨 CRITICAL: Very high fraud probability. Block and investigate immediately.")

            except Exception as e:
                st.error(f"❌ Detection failed: {str(e)}")


# ==================== SCREEN 8: SIMD OPERATIONS ====================

def screen_8_simd_operations():
    """SIMD and packed ciphertext operations"""
    st.title("🔢 SIMD Operations")
    st.markdown("### Batch Processing with Packed Ciphertexts")

    st.info("💡 SIMD (Single Instruction, Multiple Data) allows processing multiple values in parallel")
    ...
    operation = st.selectbox(
        ...
        format_func=lambda x: {
            "rotate": "Rotate Vector (circular shift)",
            "dot_product": "Dot Product (inner product)",
            "slot_wise_add": "Element-wise Addition",
            "slot_wise_multiply": "Element-wise Multiplication"
        }[x]
    )
    ...
    with col1:
        ...
        vec1 = np.random.randn(vec1_size).round(2).tolist()
        st.write(vec1)

    with col2:
        if operation in ['dot_product', 'slot_wise_add', 'slot_wise_multiply']:
            st.markdown("**Vector 2:**")
            vec2 = np.random.randn(vec1_size).round(2).tolist()
            st.write(vec2)
    ...
    if st.button("▶️ Execute SIMD Operation", type="primary"):
        with st.spinner(f"Performing {operation} on encrypted vectors..."):
            try:
                # Mock: In real implementation, vectors would be encrypted first
                encrypted_vec1 = base64.b64encode(b"mock_encrypted_vec1").decode()
                encrypted_vec2 = base64.b64encode(b"mock_encrypted_vec2").decode()

                request_data = {
                    "library": library,
                    "operation": operation,
                    "encrypted_vectors": [encrypted_vec1, encrypted_vec2] if operation != "rotate" else [
                        encrypted_vec1],
                ...
                if result and result.get('status') == 'success':
                    ...
                    # Show expected result (for verification)
                    st.divider()
                    st.subheader("Expected Result (Plaintext)")

                    if operation == "rotate":
                        rotated = vec1[steps:] + vec1[:steps]
                        st.write(f"Rotated by {steps}: {rotated}")
                    elif operation == "dot_product":
                        dot_prod = sum(v1 * v2 for v1, v2 in zip(vec1, vec2))
                        st.write(f"Dot Product: {dot_prod:.2f}")
                    elif operation == "slot_wise_add":
                        result_vec = [v1 + v2 for v1, v2 in zip(vec1, vec2)]
                        st.write(f"Element-wise Sum: {result_vec}")
                    elif operation == "slot_wise_multiply":
                        result_vec = [v1 * v2 for v1, v2 in zip(vec1, vec2)]
                        st.write(f"Element-wise Product: {result_vec}")

            except Exception as e:
                st.error(f"❌ Operation failed: {str(e)}")


# Add these screens to your main navigation
def main_enhanced():
    """Enhanced main function with new screens"""
    ...
    if page == "1. Data Upload & Config":
        ...
        screen_3_statistics()
    if page == "5. Parameter Selection":
        screen_5_parameter_selection()
    elif page == "6. ML Inference":
        screen_6_ml_inference()
    elif page == "7. Fraud Detection":
        screen_7_fraud_detection()
    elif page == "8. SIMD Operations":
