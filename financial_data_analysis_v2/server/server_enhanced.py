"""
Complete Enhanced FHE Server with:
1. Real FHE operations (no simulation)
2. Party ID/Email-based filtering
3. SIMD operations for all computations
4. Time-series analytics support
...
try:
    from tenseal_wrapper_enhanced_new import EnhancedTenSEALWrapper as TenSEALWrapper
...
try:
    from openfhe_wrapper_enhanced_new import EnhancedOpenFHEWrapper as OpenFHEWrapper
...
async def encrypt_column(request: ColumnEncryptionRequest):
    ...
    try:
        ...
        # SIMD Mode: Packed Vector (multiple values in single ciphertext)
            ...
            # Pack multiple values into single ciphertext slots
            batch_size = 128  # Pack 128 values per ciphertext
            ...
            for batch_start in range(0, len(processed_data), batch_size):
                ...
                # Remove None values for packing
                valid_data = [float(v) for v in batch_data if v is not None]

                if valid_data:
                    # Encrypt as packed vector
                    packed_encrypted = wrapper.encrypt_vector(valid_data)

                    # Store with batch metadata
                    encrypted_results.append({
                        'index': batch_start,
                        'encrypted_value': serialize_encrypted(packed_encrypted),
...
async def fraud_detection(request: FraudDetectionRequest):
    ...
    try:
        ...
        encrypted_amounts = [deserialize_encrypted(amt) for amt in request.encrypted_amounts]
        ...
        if request.detection_type == "linear_score":
            ...
            for enc_amt in encrypted_amounts[:5]:  # Use subset for performance
                # Apply weight (amount weight as primary feature)
            ...
            # Compute distance from centroid on encrypted data
            squared_diffs = []
...
async def ml_inference(request: MLInferenceRequest):
    ...
    try:
        ...
        encrypted_features = [deserialize_encrypted(f) for f in request.encrypted_features]

        if request.model_type == "linear":
            # Linear regression: y = w1*x1 + w2*x2 + ... + intercept
            print("   Computing linear regression on encrypted features...")

            prediction = None

            for enc_feature, weight in zip(encrypted_features, request.weights):
            ...
            # Apply polynomial sigmoid approximation: sigmoid(x) ≈ 0.5 + 0.197*x - 0.004*x^3
            x = linear_result
            ...
            # Use first feature for polynomial
